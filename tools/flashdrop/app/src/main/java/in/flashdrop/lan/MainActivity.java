...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        TextView title = text("Turbo 4 • Packed Folder Engine", 29, Color.WHITE, true);
        ...
        turboCard.addView(text("⚡ FlashDrop Turbo 4 for Windows",20,Color.rgb(58,48,171),true));
        TextView turboDesc=text("Turbo 4 is designed specifically for folders containing thousands of files. Each worker sends up to 500 files in one XFD2 packed batch on the same socket, removing the per-file network transaction while preserving the original folder tree and resume support.",14,Color.rgb(70,78,103),false);
        ...
        TextView exp=text("Open the FTP address above in Windows File Explorer. This remains the easiest method for normal copying. Turbo 4 keeps ordinary FTP for browsing, then uses packed XFD2 batches for high-speed folder transfer.",14,Color.rgb(56,76,72),false);
        ...
        TextView perfText=text("• XFD2 packed-folder mode: up to 500 files per network batch\n• 1–8 parallel packed streams\n• No per-file FTP RETR/EPSV cycle in Turbo mode\n• Original folder structure recreated directly on Windows\n• Resume support for partial files\n• Standard read-only FTP remains available for Explorer",13,Color.rgb(74,84,103),false);
